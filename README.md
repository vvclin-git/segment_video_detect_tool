...
按「獨立檢視當幀」會把原始 frame、分析當下的 HSV／ROI／最小面積、mask、bbox 與選取狀態完整保存成獨立視窗；播放、調參及其他視窗不會影響它。可以同時開啟多個視窗，使用滑鼠滾輪縮放、拖曳平移、「符合視窗」及「100%」。關閉所屬設定／複核視窗時，固定視窗也會釋放。

主預覽或固定視窗按「匯出 PNG」後，每次選擇一種模式：Original（預設，完整原始彩色 frame）、Mask（二值遮罩）或 Overlay（沿用預覽的遮罩疊色），再獨立決定是否包含已選 bbox。三種模式都保持原始解析度；Mask 加框時會轉成彩色。包含 bbox 時標示編號、原始像素 `x/y/width/height`，不繪製 ROI。預設檔名為 `<影片名>_frame_000123_<original|mask|overlay>_<plain|annotated>.png`；可改名、取消、覆寫確認，手動 PNG 不加入批次 manifest。
