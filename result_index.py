...
const labels={first_detection:'自動首次檢出',first_sustained_stable_start:'自動穩定起點',first_stable_confirmation:'自動穩定確認',manual_first_detection:'人工首次檢出',manual_stable_confirmation:'人工穩定確認',manual_sustained_stable_start:'人工穩定起點（回推）'};
...
function eventHtml(item){return eventRecords(item).map(e=>{const title=labels[e.event]||e.event;const frame=e.frame??'—';const reason=e.error||statusLabel(e.status);return '<div class="event"><strong>'+esc(title)+'</strong><span>F'+esc(frame)+'</span><span class="images">'+imageButton(e.raw_filename,'原圖',reason)+' '+imageButton(e.annotated_filename,'ROI／bbox標示',reason)+' '+imageButton(e.overlay_filename,'Overlay',reason)+' '+imageButton(e.mask_filename,'Mask',reason)+' <button data-focus="'+esc(e.frame??'')+'" data-card="'+esc(item.item_id)+'"'+(e.frame?'':' class="missing" disabled')+'>定位</button></span></div>'}).join('')}
